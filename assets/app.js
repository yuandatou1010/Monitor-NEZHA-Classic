...
function flagFallback(code) {
  ...
  function updateVisibleCards() {
    for (const s of state.servers) {
      ...
      if (rows[2]) rows[2].querySelector('.text-value').innerHTML = `<span class="icon-cpu">❆</span>${num(s.cpu_cores)}Core <span class="icon-ram">𓄲♒</span>${capacity(s.ram_total)}  <span class="icon-disk">▤</span>${capacity(s.disk_total)} `;
